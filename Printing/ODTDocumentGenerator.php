...
//            $nationalFrameworkModel = new NationalFramework();
            ...
            $processor->replacePattern($prefix . 'DegreeUA', $degree);
            $processor->replacePattern($prefix . 'DegreeEN', $degree === 'Бакалавр' ? 'Bachelor' : 'Master');
...
//            var_dump($estimatesModels);
        ...
        if (count($graduatesModels) === 0) {
            $content = $fileHandler->init();
            $fileHandler->createODTCopy('Test File For ONLY TESTING');
            $fileHandler->writeContentToFile($content);
            $fileHandler -> setUpODTDocument('Test File For ONLY TESTING.odt');
        }
    ...
    private function addDisciplines(int &$unitCode, array $estimatesModels, int $teaching, DisciplineDao $disciplineDao, Table $table) {
        ...
        for ($j = 0; $j < count($estimatesModels); $j++) {
            $model = $disciplineDao -> convertArrayToModels($disciplineDao->where(array('Discipline_ID', 'Teaching'), array($estimatesModels[$j] -> getDisciplineId(), $teaching), array('=', '=')));
            if (count($model) === 0) continue;
            $disciplineModel = $model[0];
